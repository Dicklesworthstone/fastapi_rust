...
- Runtime is **asupersync**, never Tokio. Handlers may take \`ctx: &RequestContext\` (or \`cx: &Cx\`); call \`ctx.checkpoint()?\` in loops and long work — \`CancelledError\` converts into \`HttpError\` (499). Raw \`cx.checkpoint()\` returns \`Result<(), asupersync::Error>\`, which has no \`HttpError\` conversion.
- \`Json<T>\` is both an extractor and a response type (\`200\`, \`application/json\`); \`Result<Json<T>, HttpError>\` is the usual handler return.
...
async fn get_item(ctx: &RequestContext, id: Path<i64>) -> Result<Json<Item>, HttpError> {
    ctx.checkpoint()?;   // cancellation-safe yield point: CancelledError -> HttpError (499)
    Ok(Json(Item { id: id.0, name: "Widget".into() }))
}
